...
const nodejs_whisper_1 = require("nodejs-whisper");
...
const FRAME_PROMPT = 'Опиши кратко, одним-двумя предложениями (максимум 200 символов), что показано на этом кадре из видео: ' +
...
function ensureMediaDir() {
    try {
        (0, fs_1.mkdirSync)(MEDIA_DIR, { recursive: true });
    }
    catch {
        // exists
    }
...
async function transcribeViaOpenAI(filePath) {
    ...
    try {
        ...
        console.error('[transcribe] stat failed:', err.message);
        return null;
    }
    ...
    const blob = await (0, fs_1.openAsBlob)(filePath);
...
async function transcribeViaServer(filePath) {
...
async function transcribeViaCli(filePath) {
    const raw = await (0, nodejs_whisper_1.nodewhisper)(filePath, {
...
async function transcribeAudio(filePath) {
    ...
    try {
        ...
        console.error(`[transcribe] ${backend} error:`, err.message);
        return null;
    }
}
...
function cleanupTempAudio(prefix) {
    try {
        const files = (0, fs_1.readdirSync)(MEDIA_DIR);
        for (const f of files) {
            if (f.startsWith(prefix)) {
                try {
                    (0, fs_1.unlinkSync)(path_1.default.join(MEDIA_DIR, f));
                }
                catch { /* ignore */ }
            }
        }
...
async function extractFrames(videoPath, prefix, durationSec) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const outPath = path_1.default.join(MEDIA_DIR, `${prefix}_frame_${i}.jpg`);
        jobs.push(execFileP('ffmpeg', [
            '-y',
            '-ss', ts.toFixed(2),
            '-i', videoPath,
            '-frames:v', '1',
            ...
            .then(async () => {
            try {
                const st = await (0, promises_1.stat)(outPath);
                if (st.size > 0)
                    frames.push({ ts, path: outPath });
            }
...
async function describeFrameViaOpenAI(b64) {
...
async function describeFrameViaAnthropic(b64) {
...
async function describeFrame(framePath, backend) {
    try {
        const b64 = (await (0, promises_1.readFile)(framePath)).toString('base64');
        ...
        console.error('[frames] vision error:', err.message);
        return null;
    }
}
...
async function analyzeFrames(videoPath, prefix, durationSec) {
    ...
    try {
        ...
        console.error('[frames] analyze error:', err.message);
        return null;
    }
}
async function processVideo(filePath, messageId) {
    ensureMediaDir();
    ...
    const audioPath = path_1.default.join(MEDIA_DIR, `vid_${messageId}.mp3`);
    try {
        ...
        const parts = [];
        if (text)
            parts.push(`[video transcription] ${text}`);
        if (frames)
            parts.push(frames);
        return parts.join('\n\n');
    }
...
async function downloadAndTranscribeAudio(url, prefix) {
    const outputTemplate = path_1.default.join(MEDIA_DIR, `${prefix}.%(ext)s`);
    ...
    const audioFile = (0, fs_1.readdirSync)(MEDIA_DIR)
        .filter(f => f.startsWith(`${prefix}.`))
        .find(f => f.endsWith('.mp3'));
    if (!audioFile)
        return null;
    return transcribeAudio(path_1.default.join(MEDIA_DIR, audioFile));
}
...
async function downloadVideo(url, prefix) {
    const outputTemplate = path_1.default.join(MEDIA_DIR, `${prefix}_v.%(ext)s`);
    try {
        await execFileP('yt-dlp', [
        ...
        const vid = (0, fs_1.readdirSync)(MEDIA_DIR)
            .filter(f => f.startsWith(`${prefix}_v.`))
            .find(f => /\.(mp4|mkv|webm|mov)$/i.test(f));
        return vid ? path_1.default.join(MEDIA_DIR, vid) : null;
    }
    catch (err) {
        console.error('[yt-dlp] video download error:', err.message);
        return null;
    }
}
async function processUrl(url, messageId) {
    ensureMediaDir();
    ...
    try {
        ...
        if (wantFrames) {
            ...
            if (videoPath) {
                gotVideo = true;
                const audioPath = path_1.default.join(MEDIA_DIR, `${cleanupPrefix}.mp3`);
                try {
        ...
        console.error('[yt-dlp] download/transcribe error:', err.message);
