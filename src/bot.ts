...
let messageCallback: ((chatId: number, text: string, username: string | null, displayName: string | null, messageId: number, replyToMessageId: number | null) => void) | null = null;
...
export function createBot(token: string): Bot {
  ...
  bot.on('message:text', async (ctx: Context) => {
    ...
    if (messageCallback) {
      messageCallback(chatId, text, username, displayName, msg.message_id, msg.reply_to_message?.message_id ?? null);
    }
  });
